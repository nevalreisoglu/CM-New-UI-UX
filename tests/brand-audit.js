...
(async () => {
  ...
  await page.selectOption('#role-sel', 'admin');
  const deep = [
    ['campaign-editor', async () => { await page.click('.nav button[data-view="campaigns"]'); await page.click('#camp-new'); }],
  ...
  console.log(`\nchecked ${checked} rendered text nodes across ${VIEWS.length} pages × 2 widths × 2 menu states, plus 5 deep states`);
  if (fails.size) {
